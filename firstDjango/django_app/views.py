...
from .models import MentalEvaluation, UserEvaluation, UserReviewForEvaluation
...
def get_user_review_list(reuqest):
    user_id = "111"  # request.POST.get()
    user_info = UserReviewForEvaluation.objects.get(user_id=user_id)
    info = []
    for item in user_info:
        mental_info = MentalEvaluation.objects.get(eval_id=item["eval_id"])
        info.append({"title": mental_info["title"], "created_on": item["created_on"]})
    user_review = {"status": HttpResponse.status_code, "msg": "success", "data": info}
...
def get_avatar(request):
    eval_id = "01"    #request.POST.get()
    info = []
    eval_info = MentalEvaluation.objects.get(eval_id=eval_id)
    info.append({"avatar": eval_info["avatar"]})
    eval = {"status": HttpResponse.status_code, "msg": "success", "data": info}
    return JsonResponse(eval)

def get_eval_detail(request):
    eval_id = "01"
    info = {"status": 200, "msg": "xxx", "data": [{"title": "xxx", "intro": "good eval", "ques_num": "30", "report": "3", "nums_eval": "3", "price": "10", "discount_price": "9", "eval_id": "1"}]}
